...
import { BookingRow } from "@/interface/BulkBookingRow";

type Props = {
  clientId: number;
};

export default function BulkBookingPage({ clientId }: Props) {
  ...
  function parseExcel(file: File) {
    ...
    reader.onload = (e) => {
      try {
        ...
        consignee_name: (r.consignee_name || r["Consignee Name"] || "").toString().trim(),
        address: (r.address || r["Address"] || "").toString().trim(),
        phone: (r.phone || r["Phone"] || "").toString().trim(),
        pincode: (r.pincode || r["Pincode"] || "").toString().trim(),
        weight: (r.weight || r["Weight"] || "").toString().trim(),
        reference: (r.reference || r["Reference"] || "").toString().trim(),
        }))
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
        <a
        href="/samples/dtdc-bulk-booking.xlsx"
        download
